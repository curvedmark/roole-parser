...
simpleSelector
	= first:(selectorInterpolation / baseSelector / suffixSelector) rest:suffixSelector* {
		rest.unshift(first);
		return rest;
	}
...
property
	= star:'*'? name:identifier _ ':' _ value:list _ priority:'!important'i? _ semicolon {
		if (star) {
...
properPostfix
	= node:primary posts:(argumentList / accessor)+ {
...
mixin
	= '@mixin'i _ val:(properPostfix / selectorList) _ semicolon {
		return {
			type: 'mixin',
			children: [val],
			loc: loc()
